...
git checkout topic/cc35xx-rev-a-launchpad-support
...
## 3. Install the SimpleLink Wi-Fi SDK

Download and run the **SimpleLink Wi-Fi SDK v9.22.00.15** Linux installer from:
https://www.ti.com/tool/download/SIMPLELINK-WIFI-SDK/9.22.00.15

Create the directory where the SDK will be placed in the Matter tree:

```bash
mkdir {matter-root}/third_party/ti_simplelink_sdk/repo_cc35xx
```

Copy the installed SDK into the Matter tree using the `-a` flag to preserve symlinks and permissions:

```bash
cp -a /path/to/sdk/. {matter-root}/third_party/ti_simplelink_sdk/repo_cc35xx
```
