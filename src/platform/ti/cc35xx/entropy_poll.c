...
//#include "ti/drivers/dpl/ClockP.h"  //TODO - how to include
extern uint32_t ClockP_getSystemTicks(void);
//extern int sha256_vector(size_t num_elem, const uint8_t *addr[], const size_t *len,uint8_t *mac);
extern int Report(const char *pcFormat,...);

int mbedtls_hardware_poll(void *data, unsigned char *output, size_t len, size_t *olen)
{
    //read clock counter
    uint32_t clk_ticks = ClockP_getSystemTicks();

    //Hash-2-BUFF 256 on clk
    uint8_t clk_hash[32];
    if (mbedtls_sha256((unsigned char *)&clk_ticks,4, clk_hash, 0) !=0){
        return MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
    }

    //Hash-2-BUFF 256 on input data
    uint8_t data_hash[32];
        if (mbedtls_sha256((unsigned char *)&data,len, data_hash, 0) !=0){
            return MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
        }

    //XOR HASH output with data
    //copy to output the first len bytes from xor result
    uint32_t loop;
    for ( loop = 0; loop < (uint32_t)len; loop++ ) {
               output[loop] = data_hash[loop] ^ clk_hash[loop];
          }
