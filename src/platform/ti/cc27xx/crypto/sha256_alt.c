...
void mbedtls_sha256_free(mbedtls_sha256_context * ctx)
{
    if (NULL == ctx)
    {
        PLAT_LOG("mbedtls_sha256_free null context");
...
{
    SHA2_HashType type;
        PLAT_LOG("mbedtls_sha256_starts_ret null context");
    ...
    if (ctx->hndl == NULL)
    {
        ...
        if (NULL == ctx->hndl)
        {
            PLAT_LOG("mbedtls_sha256_starts_ret open failed");
    ...
    if (is224)
    {
        PLAT_LOG("mbedtls_sha256_starts_ret 224");

        type = SHA2_HASH_TYPE_224;
    }
    else
    {
        PLAT_LOG("mbedtls_sha256_starts_ret 256");

        type = SHA2_HASH_TYPE_256;
    }

    SHA2_setHashType(ctx->hndl, type);
    PLAT_LOG("SHA2_setHashType");
...
int mbedtls_sha256_finish_ret(mbedtls_sha256_context * ctx, unsigned char output[32])
{
    int_fast16_t result;
    result = SHA2_
    finalize(ctx->hndl, output);

    if (SHA2_STATUS_SUCCESS != result)
    {
         PLAT_LOG("mbedtls_sha256_finish_ret failure");

        return MBEDTLS_ERR_SHA256_HW_ACCEL_FAILED;
    }
    else
    {
        PLAT_LOG("mbedtls_sha256_finish_ret success");
...
int mbedtls_sha256_update_ret(mbedtls_sha256_context * ctx, const unsigned char * input, size_t ilen)
{
    ...
    if (SHA2_STATUS_SUCCESS != result)
    {
        PLAT_LOG("mbedtls_sha256_update_ret failed");

        return MBEDTLS_ERR_SHA256_HW_ACCEL_FAILED;
    }
    else
    {
        PLAT_LOG("mbedtls_sha256_update_ret success");
...
int mbedtls_internal_sha256_process(mbedtls_sha256_context * ctx, const unsigned char data[64])
{
    ...
    if (SHA2_STATUS_SUCCESS != result)
    {
        PLAT_LOG("mbedtls_internal_sha256_process failed");

        return MBEDTLS_ERR_SHA256_HW_ACCEL_FAILED;
    }
    else
    {
        PLAT_LOG("mbedtls_internal_sha256_process success");
