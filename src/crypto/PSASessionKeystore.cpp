...
static psa_status_t hmac_sha256_psa(const uint8_t * key, size_t key_len,
                                    const uint8_t * p1, size_t p1_len,
                                    const uint8_t * p2, size_t p2_len,
                                    const uint8_t * p3, size_t p3_len,
                                    uint8_t mac[32])
{
...
static psa_status_t hkdf_extract_psa(const uint8_t * salt, size_t salt_len,
...
static psa_status_t hkdf_expand_psa(const uint8_t prk[32], const uint8_t * info, size_t info_len,
...
static psa_status_t sw_hkdf_sha256(const uint8_t * salt, size_t salt_len,
                                   const uint8_t * ikm, size_t ikm_len,
                                         ...
                                         Aes128KeyHandle & key)
{
#ifdef CHIP_CRYPTO_PSA_HKDF_IMPORT_WORKAROUND
    {
        ...
        psa_status_t psaStatus =
            sw_hkdf_sha256(salt.size() > 0 ? salt_ram : nullptr, salt.size(),
                           secret.ConstBytes(), secret.Length(),
                           info_ram, info.size(), okm, kKeyLen);
                                                 ...
                                                 AttestationChallenge & attestationChallenge)
{
    ...
    if (hkdfKey.As<psa_key_id_t>() == kHkdfRawKeyId)
    {
        ...
        psa_status_t psaStatus =
            sw_hkdf_sha256(salt.size() > 0 ? salt_ram : nullptr, salt.size(),
                           sHkdfRawKeyBytes, sHkdfRawKeyLen,
