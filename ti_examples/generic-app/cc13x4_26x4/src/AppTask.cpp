...
#include "app/icd/server/ICDNotifier.h"
 ...
 #define GENERIC_APPLICATION_IDENTIFY_ENDPOINT 1
...
//set a default value for which action to toggle to when ICD is enabled
uint8_t toggle_action = 1;
 ...
 int AppTask::Init()
 {
     ...
     if (ret != CHIP_NO_ERROR)
     {
         PLAT_LOG("ApplicationMgr().Init() failed");
         while (1)
             ;
     }
 
     ApplicationMgr().SetCallbacks(ActionInitiated, ActionCompleted);
 ...
 void AppTask::ActionInitiated(ApplicationManager::Action_t aAction, int32_t aActor)
 {
     // action initiated callback
     if (aAction == ApplicationManager::ACTION_1)
     {
         uiTurnOn();
         PLAT_LOG("ACTION_1 ActionInitiated");
     }
     else if (aAction == ApplicationManager::ACTION_2)
     {
         uiTurnOff();
         PLAT_LOG("ACTION_2 ActionInitiated");
     }
 }
 
 void AppTask::ActionCompleted(ApplicationManager::Action_t aAction)
 {
     // action completed callback
     if (aAction == ApplicationManager::ACTION_1)
     {
         uiTurnedOn();
         PLAT_LOG("ACTION_1_ActionCompleted");
     }
     else if (aAction == ApplicationManager::ACTION_2)
     {
         uiTurnedOff();
         PLAT_LOG("ACTION_2 ActionCompleted");
     }
 }
 ...
 void AppTask::DispatchEvent(AppEvent * aEvent)
 {
     ...
     case AppEvent::kEventType_ButtonLeft:
         if (AppEvent::kAppEventButtonType_Clicked == aEvent->ButtonEvent.Type)
         {
            ...
            PlatformMgr().ScheduleWork([](intptr_t) { app::ICDNotifier::GetInstance().NotifyNetworkActivityNotification(); });
             ...
             ApplicationMgr().InitiateAction(actor, ApplicationManager::ACTION_1);
#endif
         }
     ...
     case AppEvent::kEventType_ButtonRight:
         if (AppEvent::kAppEventButtonType_Clicked == aEvent->ButtonEvent.Type)
         {
            ...
            {
                ApplicationMgr().InitiateAction(actor, ApplicationManager::ACTION_2);
                toggle_action = 1;
            }
#else
             ApplicationMgr().InitiateAction(actor, ApplicationManager::ACTION_2);
