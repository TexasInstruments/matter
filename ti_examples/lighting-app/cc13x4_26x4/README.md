# Matter Lighting Example Application with AppoBLE feature
...
-   [Matter Lighting Example Application with AppoBLE feature](#matter-lighting-example-application-with-appoble-feature)
...
The lighting example has also been extended to support TI's Application over BLE
interface (AppoBLE) allowing user's to interact with custom BLE services via the
Matter application.
...
-   Run the build to produce a default executable. By default on Linux both the
    ...
    ```
    $ cd ~/connectedhomeip/ti_examples/lighting-app/cc13x4_26x4
...
The AppoBLE interface can be used to setup custom advertisements which can run
alongside the Matter advertisement set. On startup, a custom AppoBLE
advertisement set is created and started, advertising the Lighting and
Diagnostic Service 16-bit UUID's with the AppoBLE device name.

User's can commission the device onto the Matter fabric and setup the BLE
connection for AppoBLE purposes. Note: In this release on 1 BLE Connection is
supported

See src/platform/cc13xx_26xx/cc13x4_26x4/AppoBLE_interface.h for full API
definition.

User's may trigger BLE pairing/bonding out of the box by attempting to read any
of the Lighting service secured characteristics. The pairing code by default is
`111111`. Writing to the light On/Off or toggle characteristic will trigger the
application callback to update the Matter LightingManager state. Subsequently
reading/writing to the Lighting Manager via the chip tool will trigger an update
to the BLE Central automatically.

The out of box application supports sample Lighting and Diagnostic services with
the following functionality.

Lighting Service: Service UUID: 0x11A0

| Characteristic UUID | Characteristic UUID Long                                                                       | Characteristic Name                 | Characteristic Description                 | Attributes      | Type    |
| ------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------ | --------------- | ------- |
| 0x11A1              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xA1, 0x11, 0x00, 0xF0 | Light On/Off (1 for on, 0 for off)  | Set Light state to On (1) or Off (0)       | Authen R/W      | uint8_t |
| 0x11A2              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51,0x04, 0xA2, 0x11, 0x00, 0xF0  | Light Toggle (1 to toggle)          | Toggle Light on Local device(0)            | Authen R/W      | uint8_t |
| 0x11A3              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51,0x04, 0xA3, 0x11, 0x00, 0xF0  | Light State                         | Read current Light state On (1) or Off (0) | Authen R/Notify | uint8_t |

Diagnostic Service: Service UUID Short: 0x11B0 Note: User's must trigger a
refresh of the data either by triggering a periodic refresh or by writing to the
refresh characteristic.

| Characteristic UUID | Characteristic UUID Long                                                                       | Characteristic Name     | Characteristic Description            | Attributes | Type          |
| ------------------- | ---------------------------------------------------------------------------------------------- | ----------------------- | ------------------------------------- | ---------- | ------------- |
| 0x11B1              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB1, 0x11, 0x00, 0xF0 | Application Heap Free   | Read total RAM free in heap           | R          | uint64_t      |
| 0x11B2              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB2, 0x11, 0x00, 0xF0 | Application Heap Used   | Read total RAM used by heap           | R          | uint64_t      |
| 0x11B3              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB3, 0x11, 0x00, 0xF0 | Thread network key      | Read current Thread Network Key       | R          | uint8_t [16]  |
| 0x11B4              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB4, 0x11, 0x00, 0xF0 | Thread Network PAN ID   | Read Current Thread Network PAN ID    | R          | uint16_t      |
| 0x11B5              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB5, 0x11, 0x00, 0xF0 | Thread Network channel  | Read current Thread Network Channel   | R          | uint8_t       |
| 0x11B6              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB6, 0x11, 0x00, 0xF0 | Refresh Data            | Trigger Diagnostic Data refresh       | W          | uint16_t      |
| 0x11B7              | 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x40, 0x51, 0x04, 0xB7, 0x11, 0x00, 0xF0 | Refresh Period          | Trigger automatic refresh at interval | W          | uint8_t       |

[TI OAD Service][ble_user_guide_oad_service_desc]:

Note: Only Off-chip BLE OAD is currently supported

The flag to enable or disable the OTA BLE feature is determined by
"enable_appoble_ota" in the application's args.gni file.

The image required to be transferred to the Matter device differs between Matter
OTA and BLE OAD. For Matter OTA, the _-example.ota file, which contains the
Matter specific OTA header, must be used. For BLE OAD, _-example_header.bin file
which excludes the Matter specific OTA header, must be used.

If enabled, the application will wait for the central to start the begin
sequence that will instruct the central to send blocks to the peripheral device.

User's may use the either the Simplelink Connect Phone Application or Btool to
perform the BLE OAD.

If using the [TI Simplelink Connect][ble_simplelink_connect_app] Application:

1. Connect to the target peripheral device and pair
2. Select "Update FW"
3. Either choose a default Matter image from the TI github repository, or add a
   local image
4. Select update
5. Disconnect from the device to trigger an automatic reset or reset the device
   manually to boot into the new image

If using Btool, refer to the [BTool User's Guide][ble_user_guide_host_test] for
more information.

Refer to the [BLE OAD User's Guide][ble_users_guide_oad] for more information
regarding the BLE OAD service.
...
[ble_users_guide_oad]:
    https://dev.ti.com/tirex/explore/content/simplelink_cc13xx_cc26xx_sdk_7_41_00_17/docs/ble5stack/ble_user_guide/html/ble-stack-5.x-guide/ble5-oad-index-mcuboot.html#mcuboot-over-the-air-download-oad
[ble_user_guide_host_test]:
    https://dev.ti.com/tirex/explore/content/simplelink_cc13xx_cc26xx_sdk_7_41_00_17/docs/ble5stack/ble_user_guide/html/ble-stack-5.x/testing-with-btool-cc26xx.html?highlight=btool#rf-testing-using-btool
[ble_user_guide_oad_service_desc]:
    https://dev.ti.com/tirex/explore/content/simplelink_cc13xx_cc26xx_sdk_7_41_00_17/docs/ble5stack/ble_user_guide/html/ble-stack-oad/oad-profile.html?highlight=btool#oad-service-0xffc0
[ble_simplelink_connect_app]:
    https://www.ti.com/tool/SIMPLELINK-CONNECT-SW-MOBILE-APP#overview
