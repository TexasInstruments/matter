...
#ifdef TI_APPOBLE_ENABLE
void AppoBLEDiagRefresh(void);
void throughput_calc(ThroughputRate_t * throughput_data);
extern uint8_t upstream_flag;
extern ThroughputRate_t upstream_calc;
extern ThroughputRate_t downstream_calc;
...
#ifdef TI_APPOBLE_ENABLE
    /*
     * Generate new deivce name, NOTE: This will overwrite Matter Advertisement name
     * and must be changed back for Spec compliant Matter advertisements.
     */
    // const char * deviceName = "TestName-BLE";
    // ConnectivityMgr().SetBLEDeviceName(deviceName);

    /* Update Pairing Code */
    uint32_t pairingCode = 111111;
    AppoBLE().SetAppoBLEPairingPassCode(pairingCode);

    uint8_t advGenStatus = AppoBLE().AddUpdateAppoBLEAdvSet(ADV_INDEX_INVALID, advParams2, advData2, sizeof(advData2), scanResData2,
                                                            sizeof(scanResData2), (GapAdv_eventMaskFlags_t) (0));
    if (advGenStatus == 0)
    {
        PLAT_LOG("AppoBLE Adv request successful ");
    }
#endif
    return 0;
}
...
{
    PLAT_LOG("Lighting Action completed Callback");
#ifdef TI_APPOBLE_ENABLE
    BLE_CONNECTION_OBJECT conId;
    uint8_t lightstate;

    /* Obtain first connection handle */
    conId = AppoBLE().AppoBLE_GetValidConnection(1);

    if (conId != NULL)
    {
        PLAT_LOG("First Active Connection ID: %d", *((uint16_t *) conId));
    }
#endif
    if (aAction == LightingManager::ON_ACTION)
    {
...
#ifdef TI_APPOBLE_ENABLE
        lightstate = 1;
        AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) LightServiceServUUID, (ChipBleUUID *) LightServiceLightStateUUID,
                                          &lightstate, sizeof(lightstate));
#endif
    }
    else if (aAction == LightingManager::OFF_ACTION)
    {
...
void AppTask::DispatchEvent(AppEvent * aEvent)
{
    ...
    case AppEvent::kEventType_ButtonLeft:
        ...
        {
#ifdef TI_APPOBLE_ENABLE
            if (AppoBLE().ScheduleAppoBLEFxn(eraseAllBonds, NULL) == SUCCESS)
            {
                PLAT_LOG("Successfully scheduled custom function");
            }
#endif
    ...
    case AppEvent::kEventType_ButtonRight:
        if (AppEvent::kAppEventButtonType_Clicked == aEvent->ButtonEvent.Type)
        {
            ...
            PLAT_LOG("DispatchEvent Entry off action");
    ...
    case AppEvent::kEventType_AppEvent:
        ...
        break;
#ifdef TI_APPOBLE_ENABLE
    case AppEvent::kEventType_Diagnostic_refresh:
        PLAT_LOG("Diagnostics being refreshed");
        AppoBLEDiagRefresh();
        break;

    case AppEvent::kEventType_Diagnostic_timer:
        PLAT_LOG("Periodic diagnostic refresh");
        AppoBLEDiagRefresh();
        break;

    case AppEvent::kEventType_Throughput_Matter_Disable:
        PLAT_LOG("Locking matter and thread");
        matterThreadDisable();
        break;

    case AppEvent::kEventType_Throughput_Matter_Enable:
        PLAT_LOG("Unlocking matter and thread");
        matterThreadEnable();
        break;
    case AppEvent::kEventType_Throughput_calculate:
        if (upstream_flag)
        {
            throughput_calc(&upstream_calc);
        }
        else
        {
            throughput_calc(&downstream_calc);
        }
        break;
#endif
...
void AppoBLEDiagRefresh(void)
{
    ...
    // handle getting diagnostic info
    panId = ThreadStackMgrImpl().GetPanID();
    ThreadStackMgrImpl().GetNetworkKey(netKey);
    threadChannel = ThreadStackMgrImpl().GetChannel();
    GetDiagnosticDataProvider().GetCurrentHeapUsed(heapUsed);
    GetDiagnosticDataProvider().GetCurrentHeapFree(heapFree);
    ...
    AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) diagnosticServiceUUID, (ChipBleUUID *) diagnosticService_HeapFreeUUID,
                                      (uint8_t *) &heapFree, sizeof(heapFree));

    AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) diagnosticServiceUUID, (ChipBleUUID *) diagnosticService_HeapUsedUUID,
                                      (uint8_t *) &heapUsed, sizeof(heapUsed));

    AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) diagnosticServiceUUID,
                                      (ChipBleUUID *) diagnosticService_ThreadNetKeyUUID, (uint8_t *) &netKey, sizeof(netKey));

    AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) diagnosticServiceUUID,
                                      (ChipBleUUID *) diagnosticService_ThreadChannelUUID, (uint8_t *) &threadChannel,
                                      sizeof(threadChannel));

    AppoBLE().SendAppoBLEWriteRequest(conId, (ChipBleUUID *) diagnosticServiceUUID,
                                      (ChipBleUUID *) diagnosticService_ThreadPanIDUUID, (uint8_t *) &panId, sizeof(panId));
}

void throughput_calc(ThroughputRate_t * throughput_data)
{
    uint16_t instant_total = 0;
    for (uint16_t i = 0; i < throughput_data->message_count; i++)
    {
        instant_total += throughput_data->stored_bytes[i];
    }
    // reset message count for next calc period
    throughput_data->message_count = 0;
    // store instant rate and increment the index
    throughput_data->bytes_cb[throughput_data->bytes_cb_index] = instant_total;

    if (throughput_data->bytes_cb_index == (CB_SIZE - 1))
    {
        throughput_data->cbBufferFilled = TRUE;
        throughput_data->bytes_cb_index = 0;
    }
    else
    {
        throughput_data->bytes_cb_index++;
    }

    // calculate average rate
    uint16_t avg        = 0;
    uint8_t msg_samples = 0;
    // check if buffer is full or not
    if (throughput_data->cbBufferFilled)
    {
        for (int i = 0; i < CB_SIZE; i++)
        {
            avg += throughput_data->bytes_cb[i];
        }
        avg         = avg / CB_SIZE;
        msg_samples = CB_SIZE;
    }
    ...
    {
        for (int i = 0; i < throughput_data->bytes_cb_index; i++)
        {
            avg += throughput_data->bytes_cb[i];
        }
        avg         = avg / (throughput_data->bytes_cb_index);
        msg_samples = throughput_data->bytes_cb_index;
    }
    avg = avg * 8;
    PLAT_LOG("Number of messages: %d", throughput_data->log_message_count);
    PLAT_LOG("average throughput rate (kb/s): %d.%d, over %d samples", (avg / 1000), (avg % 1000), msg_samples);
}
#endif