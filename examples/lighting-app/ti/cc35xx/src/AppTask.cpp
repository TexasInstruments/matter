...
#include <platform/ti/cc35xx/OTAImageProcessorImpl.h>
#endif
...
#include <src/platform/ti/cc35xx/DefaultTestEventTriggerDelegate.h>
...
#define APP_TASK_STACK_SIZE (9 * 1024)
#define APP_TASK_PRIORITY 4
#define APP_EVENT_QUEUE_SIZE 10
#define AUTO_PRINT_METRICS 0
...
#define OPEN_COMMISSIONING_WINDOW_DURATION 300
...
int AppTask::Init()
{
    ...
    static DefaultTestEventTriggerHandler sTestEventTriggerHandler;
    VerifyOrDie(sTestEventTriggerDelegate.AddHandler(&sTestEventTriggerHandler) == CHIP_NO_ERROR);
    initParams.testEventTriggerDelegate = &sTestEventTriggerDelegate;

    // Use PSAOperationalKeystore since PSA crypto implementation (chip_crypto = PSA) is used for CC35xx
    ...
    // Register the NetworkCommissioning cluster so its FeatureMap and other
    // EXTERNAL_STORAGE attributes are served by the cluster implementation
    // rather than falling through to ember (which returns kFailure for them).
    static chip::app::Clusters::NetworkCommissioning::Instance sNetCommInstance(
        0, &chip::DeviceLayer::NetworkCommissioning::CC35XXWiFiDriver::GetInstance());
    LogErrorOnFailure(sNetCommInstance.Init());
    ...
    if (Server::GetInstance().GetFabricTable().FabricCount() == 0)
    {
        if (Server::GetInstance().GetCommissioningWindowManager().OpenBasicCommissioningWindow(
                System::Clock::Seconds32(OPEN_COMMISSIONING_WINDOW_DURATION)) == CHIP_NO_ERROR)
        {
            PLAT_LOG("Opened Matter Commissioning Window");
        }
        else
        {
...
#else
        if (xQueueReceive(sAppEventQueue, &event, pdMS_TO_TICKS(5000)) == pdTRUE)
        {
            sAppTask.DispatchEvent(&event);
        }
        else
...
void printStackAndHeapUsage(void)
{
    ...
    {
        PLAT_LOG("Task: %s, Minimum free stack space: %u bytes", pxTaskStatusArray[x].pcTaskName,
    ...
    PLAT_LOG("Current free heap space: %u bytes", (unsigned int) xFreeHeapSpace);
    PLAT_LOG("Minimum ever free heap space: %u bytes", (unsigned int) xMinimumEverFreeHeapSpace);
}
#endif
...
void AppTask::uiInit(void)
{
    ...
    sAppRedHandle = LED_open(CONFIG_LED_0, &ledParams);
    LED_setOff(sAppRedHandle);

    LED_Params_init(&ledParams); // default PWM LED
    sAppGreenHandle = LED_open(CONFIG_LED_1, &ledParams);
    LED_setOff(sAppGreenHandle);
#endif // LED ENABLE
    ...
    sAppLeftHandle                 = Button_open(CONFIG_BUTTON_0, &buttonParams);
    Button_setCallback(sAppLeftHandle, ButtonLeftEventHandler);

    Button_Params_init(&buttonParams);
    buttonParams.buttonEventMask   = Button_EV_CLICKED | Button_EV_LONGCLICKED;
    buttonParams.longPressDuration = 1000U; // ms
    sAppRightHandle                = Button_open(CONFIG_BUTTON_1, &buttonParams);
