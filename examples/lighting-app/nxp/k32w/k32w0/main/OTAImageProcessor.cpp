...
CHIP_ERROR OTAImageProcessor::ProcessBlock(ByteSpan & block)
{
...
void OTAImageProcessor::HandlePrepareDownload(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<OTAImageProcessor *>(context);
...
void OTAImageProcessor::HandleAbort(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<OTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
void OTAImageProcessor::HandleProcessBlock(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<OTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
CHIP_ERROR OTAImageProcessor::SetBlock(ByteSpan & block)
{
    if ((block.data() == nullptr) || block.empty())
    {
        return CHIP_NO_ERROR;
...
void OTAImageProcessor::HandleFinalize(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<OTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
CHIP_ERROR OTAImageProcessor::ReleaseBlock()
{
    if (mBlock.data() != nullptr)
    {
