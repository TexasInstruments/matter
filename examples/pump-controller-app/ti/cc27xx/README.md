...
The CC27xx pump controller example application provides a working demonstration
of a connected pump controller device. This uses the open-source Matter
implementation and the Texas Instruments SimpleLink™ F3 software development
kit.

This example is enabled to build for CC2745R10-Q1 devices.
...
| Action                                                  | Functionality                                  |
| ------------------------------------------------------- | ---------------------------------------------- |
| Right Button (`BTN-2`) Press (more than 1000 ms)        | User Active Mode Trigger (part of LIT support) |
...
-   If building for the CC2755P20 LaunchPad, in `args.gni` modify the field
    shown below.

    ```
    ti_simplelink_board = LP_EM_CC2755P20
    ```

-   Run the build to produce a default executable. By default on Linux both the
    TI SimpleLink SDK and Sysconfig are located in a `ti` folder in the user's
    home directory, and you must provide the absolute path to them. For example
    `/home/username/ti/sysconfig_1.23.2`. On Windows the default directory is
    `C:\ti`. Take note of this install path, as it will be used in the next
    step.

    ```
    $ cd ~/connectedhomeip/examples/pump-controller-app/ti/cc27xx
    $ gn gen out/debug --args="ti_sysconfig_root=\"$HOME/ti/sysconfig_1.23.2\""
    $ ninja -C out/debug
    ...
    ```
    gn gen out/debug --args="ti_sysconfig_root=\"$HOME/ti/sysconfig_1.23.2\" target_defines=[\"TI_ATTESTATION_CREDENTIALS=1\"] chip_generate_link_map_file=true"
